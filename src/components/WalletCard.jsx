...
export default function WalletCard({ coinType, wallet, onCopy }) {
  ...
  const handleCopy = (text, keyType) => {
    ...
    setCopied(keyType);
    toast.success(
      `${keyType === "public" ? "Public" : "Private"} key copied!`,
      {
        position: "top-right",
        autoClose: 2000,
    ...
    <div className="bg-white/5 rounded-xl p-4 backdrop-blur-sm hover:bg-white/10 transition-all border border-white/10 shadow-sm">
      ...
      <div className="flex justify-between items-center">
        <div className="flex items-center">
          ...
          <h4 className="text-matisse-100">
            {coinType.charAt(0).toUpperCase() + coinType.slice(1)} Wallet #
            {wallet.index}
          </h4>
        </div>
        {/* Reveal/Hide Button */}
        ...
        >
          {showPrivateKey ? (
            <LockOpenIcon className="h-4 w-4 text-matisse-300" />
          ) : (
            <LockClosedIcon className="h-4 w-4 text-matisse-300" />
          )}
        </button>
      </div>
      ...
      <div className="mt-3">
        <div className="flex justify-between items-center mb-1">
          <span className="text-xs ml-1 text-matisse-200">Public Key</span>
          <button
